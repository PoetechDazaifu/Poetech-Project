...
監視、セキュリティヘッダー、レート制限、ログ・個人情報の運用方針は [docs/operations.md](docs/operations.md) を参照してください。

Cloudflareの導入判断、段階計画、ロールバック方法は [docs/cloudflare-plan.md](docs/cloudflare-plan.md) を参照してください。

検索キーワードが3文字以上の場合はSQLite FTS5のtrigram索引を使い、2文字以下は日本語の短い語も扱えるよう部分一致検索へフォールバックします。データ元は「ポスト」「句会」「広報」の3分類で絞り込めます。AIタグと居住地分類の候補・件数は、登録済みデータから画面へ動的に表示されます。
